...
contract EIP712Proxy is Semver, EIP712 {
    ...
    ///         deadline: 1673891048
    ///     }])
    function multiAttestByDelegation(MultiDelegatedProxyAttestationRequest[] calldata multiDelegatedRequests)
        public
        payable
        virtual
    ...
    ///         deadline: 1673891048
    ///     }])
    function multiRevokeByDelegation(MultiDelegatedProxyRevocationRequest[] calldata multiDelegatedRequests)
        public
        payable
        virtual
    {
    ...
    function _verifyRevoke(DelegatedProxyRevocationRequest memory request) internal {
        ...
        bytes32 digest = _hashTypedDataV4(
            keccak256(
                abi.encode(
                    REVOKE_PROXY_TYPEHASH, request.revoker, request.schema, data.uid, data.value, request.deadline
                )
            )
        );
